...
const count = (target, input) => {
  return input.reduce((acc, arr) => arr.filter((num) => num === target).length + acc, 0);

  // return input.filter((num) => num === target).reduce((acc, val) => acc + val.length, 0);
  //use return array.length? 
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((arr) =>  arr.filter(val => (val) && !(val %5)).map((val) => Math.pow(2, val)));
...
let findShortest = (data) => {
  let shortName = data.reduce((acc, obj) => obj.map((name) => name.value.filter(name.length));
  return shortName[0];
};
